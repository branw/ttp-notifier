...
using an obtuse waitlisting system. This tool, with a little setup, will notify
you by e-mail whenever an appointment slot opens for your location. It
leverages free GitHub Actions to automatically run a Python script at an
interval, which then fails whenever an appointment is found; GitHub will then
notify you of the workflow failure by email shortly after. 
...
1. Fork this repository: https://github.com/branw/ttp-notifier/fork
2. In your fork, go to **Settings** > **Security** > **Secrets and Variables**
   \> **Actions** and click the **Variables** tab:
   [https://github.com/you/your-fork/settings/variables/actions](../../settings/variables/actions)
3. Gather the IDs for all the locations you are interested in [using the
   table below](#locations-reference).
4. Click the **New repository variable** button and enter:
   - **Name**: `LOCATION_IDS`
   - **Value**: Enter the location IDs from step 3, separated by commas. For
     example, `5140,6480` to check both JFK and the US Customs House in NYC.
...
7. Sit back and wait until you get a "Run failed" email from GitHub.
...
(Generated by `src/print_locations_table.py`)

Use these tables to find the IDs of your nearest CBP locations.
